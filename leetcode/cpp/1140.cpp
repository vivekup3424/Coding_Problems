...
public:
    ...
    {
        if (i == piles.size())
            return 0; // We reach the end of piles.
        if (2 * M >= piles.size() - i)
            return sums[i]; // If 2*M is equal of more of the rest of the
                            // piles then the player takes them all because
                            // every player plays optimally.
        if (dp[i][M])
            return dp[i][M]; // We might have been in this stage before with the same or
