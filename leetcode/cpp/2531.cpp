...
public:
    ...
    {                   // made this helper fxn for easy insert/ removal from map
        mp[toInsert]++; // increment freq for char to be inserted
        mp[toRemove]--; // decrement freq for char to be removed
        if (mp[toRemove] == 0)
            mp.erase(toRemove); // if freq of that char reaches zero, then remove the key from map
    }
    ...
    bool isItPossible(string word1, string word2)
    {
        ...
        for (char w1 : word1)
            mp1[w1]++; // store freq of chars in word1 in mp1
        for (char w2 : word2)
            mp2[w2]++; // store freq of chars in word2 in mp2
        ...
        for (char c1 = 'a'; c1 <= 'z'; c1++)
        {
            for (char c2 = 'a'; c2 <= 'z'; c2++)
            {
                if (mp1.count(c1) == 0 || mp2.count(c2) == 0)
                    continue; // if any of the char is not present then skip
                ...
                if (mp1.size() == mp2.size())
                    return true; // if size of both maps are equal then possible return true
