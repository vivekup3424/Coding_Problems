...
async function introspectToken(token: string): Promise<{ active: boolean;[key: string]: any }> {
    try {
        ...
        // Try to get site_id from our mapping using token identifier
        const tokenId = getTokenIdentifier(token);
        const siteId = tokenSiteMapping.get(tokenId);
        
        if (siteId) {
            response.data.site_id = siteId;
            console.log(`Retrieved site_id from mapping: ${tokenId} -> ${siteId}`);
        } else {
            console.log(`No site_id found in mapping for token: ${tokenId}`);
        }
...
async function validateToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const userInfo = {
            ...
            site_id: introspectionResult.site_id || decodedToken.site_id,
...
function generateServiceUserJWTAssertion(siteId?: string): string {
    ...
    // Using a namespaced claim to ensure it's preserved
    if (siteId) {
        payload.site_id = siteId;
        payload['urn:zitadel:iam:user:metadata'] = {
            site_id: siteId
        };
    }
...
async function getServiceUserAccessToken(jwtAssertion: string, siteId?: string): Promise<any> {
    try {
        ...
        // Store site_id mapping if provided
        if (siteId && accessToken) {
            const tokenId = getTokenIdentifier(accessToken);
            tokenSiteMapping.set(tokenId, siteId);
            console.log(`Stored site_id mapping: ${tokenId} -> ${siteId}`);
        }
...
app.post('/api/validate-token', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Log site_id if found
        if (validationResult.site_id) {
            console.log(`Token validation completed for site_id: ${validationResult.site_id}`);
        }
...
// Token decode endpoint (for debugging and site_id extraction)
app.post('/api/decode-token', (req: Request, res: Response): void => {
    try {
        const { token } = req.body;
        ...
        // Try to decode JWT, but handle opaque tokens gracefully
        const decodedToken = jwt.decode(token) as any;
        
        // Get site_id from our mapping using token identifier
        const tokenId = getTokenIdentifier(token);
        const siteIdFromMapping = tokenSiteMapping.get(tokenId);

        if (!decodedToken) {
            // This might be an opaque token, return info from our mapping
            res.json({
                success: true,
                message: 'Token processed (opaque token detected)',
                data: {
                    token_type: 'opaque',
                    site_id: siteIdFromMapping,
                    has_site_id: !!siteIdFromMapping,
                    token_format: 'opaque',
                    token_identifier: tokenId,
                    note: 'This is an opaque token. Site ID retrieved from server mapping.'
                }
            });
            return;
        }

        // JWT token - extract information normally
        const siteId = decodedToken.site_id || siteIdFromMapping;
        ...
        res.json({
            ...
            data: {
                ...
                site_id: siteId,
                has_site_id: !!siteId,
                token_type: decodedToken.token_type || 'access_token',
                issued_at: decodedToken.iat ? new Date(decodedToken.iat * 1000).toISOString() : null,
                expires_at: decodedToken.exp ? new Date(decodedToken.exp * 1000).toISOString() : null,
...
// In-memory store for site_id to token mapping
const tokenSiteMapping = new Map<string, string>();

// Helper function to generate a unique token identifier
function getTokenIdentifier(token: string): string {
    // Extract the last 20 characters as a unique identifier
...
app.listen(PORT, () => {
    ...
    console.log('   POST /api/decode-token - Decode token and extract site_id');
    console.log('   POST /api/test-site-token - Test full token lifecycle with site_id');
...
// Test endpoint for full token lifecycle with site_id
app.post('/api/test-site-token', async (req: Request, res: Response): Promise<void> => {
    try {
        const { site_id } = req.body;
        ...
        console.log(`\n🧪 Testing token lifecycle for site_id: ${trimmedSiteId}`);

        // Step 1: Generate token
        const jwtAssertion = generateServiceUserJWTAssertion(trimmedSiteId);
        const tokenResponse = await getServiceUserAccessToken(jwtAssertion, trimmedSiteId);
        
        console.log(`✅ Token generated for site_id: ${trimmedSiteId}`);

        // Step 2: Decode token (handle both JWT and opaque tokens)
        const decodedToken = jwt.decode(tokenResponse.access_token) as any;
        const tokenId = getTokenIdentifier(tokenResponse.access_token);
        const decodedSiteId = decodedToken?.site_id || tokenSiteMapping.get(tokenId);

        // Step 3: Validate token
        const validationResult = await introspectToken(tokenResponse.access_token);
        const validatedSiteId = validationResult.site_id;

        console.log(`🔍 Site ID found in decoded token: ${decodedSiteId || 'Not found'}`);
        console.log(`🔍 Site ID found in validation: ${validatedSiteId || 'Not found'}`);

        res.json({
            success: true,
            message: 'Token lifecycle test completed',
            data: {
                requested_site_id: trimmedSiteId,
                token_info: {
                    token_identifier: tokenId,
                    token_type: decodedToken ? 'jwt' : 'opaque',
                    access_token: tokenResponse.access_token
                },
                token_generation: {
                    success: true,
                    site_id_in_response: tokenResponse.site_id,
                    token_type: tokenResponse.token_type,
                    expires_in: tokenResponse.expires_in
                },
                token_decode: {
                    site_id_found: !!decodedSiteId,
                    site_id_value: decodedSiteId,
                    token_subject: decodedToken?.sub,
                    token_issuer: decodedToken?.iss,
                    is_jwt: !!decodedToken
                },
                token_validation: {
                    is_active: validationResult.active,
                    site_id_found: !!validatedSiteId,
                    site_id_value: validatedSiteId,
                    validation_subject: validationResult.sub
                },
                site_id_consistency: {
                    all_match: trimmedSiteId === decodedSiteId && decodedSiteId === validatedSiteId,
                    requested: trimmedSiteId,
                    decoded: decodedSiteId,
                    validated: validatedSiteId
                }
            }
        });

    } catch (error) {
        console.error('Token lifecycle test error:', (error as Error).message);
        res.status(500).json({
            error: 'Token lifecycle test failed',
            message: (error as Error).message
        });
    }
});

// Token mapping info endpoint (for debugging)
app.get('/api/token-mappings', (req: Request, res: Response): void => {
    const mappings = Array.from(tokenSiteMapping.entries()).map(([tokenId, siteId]) => ({
        token_identifier: tokenId,
        site_id: siteId
    }));
    ...
    res.json({
        success: true,
        message: 'Token mappings retrieved',
        data: {
            mapping_count: tokenSiteMapping.size,
            mappings: mappings
        }
    });
});
...
app.listen(PORT, () => {
    ...
    console.log('   POST /api/decode-token - Decode token and extract site_id');
    console.log('   POST /api/test-site-token - Test full token lifecycle with site_id');
    console.log('   GET  /api/token-mappings - View current token mappings');
    console.log('   GET  /api/service-info - Get service information');
    console.log('   GET  /api/zitadel/org/me - Get organization info via ZITADEL API');
    console.log('\n🔐 Authentication:');
    console.log('   🔍 Token Introspection: JWT client assertion with API grants');
